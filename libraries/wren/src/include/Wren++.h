...
public:
    ...
    T* ptr() {
       if( type() != std::type_index( typeid(T) ) )
          throw std::runtime_error( "type mismatch, expected " + boost::core::demangle(typeid(T).name()) + ", but received " + boost::core::demangle(type().name()) );
...
/*
 * Wraps a pointer to a class object. The lifetimes of the pointed-to objects are managed by the  host program.
 */
template<typename T>
...
struct WrenSlotAPI {
    static T get( WrenVM* vm, int slot ) {
        if(  WREN_TYPE_FOREIGN  != wrenGetSlotType(vm,slot) ) 
           throw std::runtime_error( "invalid type, expected foreign class" );
...
struct WrenSlotAPI< T& > {
    static T& get(WrenVM* vm, int slot) {
        if(  WREN_TYPE_FOREIGN  != wrenGetSlotType(vm,slot) ) 
           throw std::runtime_error( "invalid type, expected " + boost::core::demangle( typeid(T).name() ) + "&" );
        ForeignObject* obj = static_cast<ForeignObject*>(wrenGetSlotForeign( vm, slot ));
        return *(obj->ptr<T>());
    }
...
struct WrenSlotAPI< const T& > {
    static const T& get(WrenVM* vm, int slot) {
        if(  WREN_TYPE_FOREIGN  != wrenGetSlotType(vm,slot) ) 
           throw std::runtime_error( "invalid type, expected const " + boost::core::demangle( typeid(T).name() ) + "&"  );
        ForeignObject* obj = static_cast<ForeignObject*>(wrenGetSlotForeign( vm, slot ));
        return *(obj->ptr<T>());
    }
...
struct WrenSlotAPI< const T* > {
    static const T* get(WrenVM* vm, int slot) {
        if(  WREN_TYPE_FOREIGN  != wrenGetSlotType(vm,slot) ) 
           throw std::runtime_error( "invalid type, expected const " + boost::core::demangle( typeid(T).name() ) + "*" );
        ForeignObject* obj = static_cast<ForeignObject*>(wrenGetSlotForeign( vm, slot ));
        return static_cast< const T* >(obj->ptr<T>());
    }
...
struct WrenSlotAPI< int > {
    static int get( WrenVM* vm, int slot ) {
        if(  WREN_TYPE_NUM  != wrenGetSlotType(vm,slot) ) 
           throw std::runtime_error( "invalid type, expected int" );
...
struct WrenSlotAPI< const char* > {
    static const char* get( WrenVM* vm, int slot ) {
        if(  WREN_TYPE_STRING != wrenGetSlotType(vm,slot) ) 
           throw std::runtime_error( "invalid type, expected string (const char*)" );
...
struct WrenSlotAPI< std::string > {
    static std::string get( WrenVM* vm, int slot ) {
        if(  WREN_TYPE_STRING != wrenGetSlotType(vm,slot) ) 
           throw std::runtime_error( "invalid type, expected string (std::string)" );
        int size = 0;
        const char* bytes = wrenGetSlotBytes( vm, slot, &size );
        return std::string( bytes, size ); //wrenGetSlotString( vm, slot ) );
    }

    static void set( WrenVM* vm, int slot, const std::string& str ) {
        wrenSetSlotBytes( vm, slot, str.c_str(), str.size() );
    }
};
...
struct WrenSlotAPI< const std::string& > {
    static const char* get(WrenVM* vm, int slot) {
        if(  WREN_TYPE_STRING != wrenGetSlotType(vm,slot) ) 
           throw std::runtime_error( "invalid type, expected string (const std::string&)" );
        std::cerr <<"warning get const std::string... will truncate at null";
        return wrenGetSlotString(vm, slot);
    }
...
void propertyGetter( WrenVM* vm ) {
    ForeignObject* objWrapper = static_cast<ForeignObject*>(wrenGetSlotForeign(vm, 0));
    const T* obj = static_cast<const T*>(objWrapper->ptr<T>()); //objectPtr());
...
void propertySetter( WrenVM* vm ) {
    ForeignObject* objWrapper = static_cast<ForeignObject*>(wrenGetSlotForeign(vm, 0));
    T* obj = static_cast<T*>(objWrapper->ptr<T>());
